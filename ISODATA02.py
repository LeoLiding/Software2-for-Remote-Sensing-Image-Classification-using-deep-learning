...
#datadet = gdal.Open("E:\\colleague\\三\\遥感\\影像分类\\Kmeans\\or_196560080.tif")
path="E:\\colleague\\三\\遥感\\影像分类\\Kmeans\\or_196560080.tif"
outputFilename = "ISODATA_out3.tif"

argvK = 5#初始类别数（期望）
argvTN = 30#每个类别中样本最小数目
argvTS = 1#每个类别的标准差
...
def my_main(path, outputfilename,K: int, TN: int, TS: float, TC:float, L: int, I: int):   
    ...
    # 随机生成聚类中心
    for i in range(0, K):
        randomX = random.randint(0, imgX)
        randomY = random.randint(0, imgY)
        duplicated = False
        ...
        if not duplicated:
            clusterList.append(Cluster(numpy.array([imgArray[0,randomX, randomY],
                                                    ...
                                                    dtype = numpy.uint8)))
    print("clusterList.shape",len(clusterList))
    # 开始迭代
    iterationCount = 0
    ...
    # data = gdal.Open(outputfilename)  # 读取tif文件
...
if __name__ == '__main__':
    main(path,outputFilename,argvK,argvTN,argvTS,argvTC,argvL,argvI)
